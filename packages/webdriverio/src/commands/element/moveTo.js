...
export default async function moveTo ({ xOffset, yOffset } = {}) {
    ...
    const newXOffset = parseInt(x + (typeof xOffset === 'number' ? xOffset : (width / 2)) - scrollX, 10)
    const newYOffset = parseInt(y + (typeof yOffset === 'number' ? yOffset : (height / 2)) - scrollY, 10)
